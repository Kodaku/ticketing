...
import { BadRequestError, NotFoundError, OrderStatus, requireAuth, validateRequest } from "@amtickets2/common";
...
router.post('/api/orders', requireAuth, [
    body('ticketId')
        .not()
        .isEmpty()
        .custom((input: string) => mongoose.Types.ObjectId.isValid(input)) //make sure user is providing a mongoID
 ...
 async (req: Request, res: Response) => {
    ...
    if(isReserved) {
        throw new BadRequestError('Ticket is already created');
    }
